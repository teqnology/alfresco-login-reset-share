		...
		function main()
		{
		   ...
		   if (email.indexOf("@") > -1){
		      ...
		      if (users.length > 1) 
		      {
		         ...
		         status.setCode(status.STATUS_BAD_REQUEST, "Multiple users found. Reload this page and try to use one of the following: " + usersArray.toString());
		         status.redirect = true;
		         return;
		      }
		   ...
		   {
		      status.setCode(status.STATUS_INTERNAL_SERVER_ERROR, "The email with the new password was not set. Please reload this page and retry or contact your system administrator");
		      status.redirect = true;
		      return;
		   }
